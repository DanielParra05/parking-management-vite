...
export default defineComponent({
  ...
  methods: {
    ...
    createParkingTicket(event: Event) {
      if (
        (this.mainStore.getCarSpots.length < 20 &&
          this.parkingTicket.vehicleType === "Car") ||
        (this.mainStore.getBikeSpots.length < 10 &&
        ...
        ApiConsumer.createParkingTicket(
          ...
          this.parkingTicket.vehicleType === "Car"
            ? this.mainStore.fillCarSpot(this.parkingTicket)
            : this.mainStore.fillBikeSpot(this.parkingTicket);
          this.showDialog(
